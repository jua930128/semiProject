package com.kh.semi.common.util;

public class PageInfo {
	// 현재 페이지
	private int currentPage;
	
	// 한 페이지에 보이는 페이지의 수 : 5개
	 ...
	 * @param totalCount 전체 리스트의 수 (listCount -> totalCount로 변경)
	 * @param listLimit 한 페이지에 표시될 리스트의 수
	 */
	public PageInfo(int currentPage, int pageLimit, int totalCount, int listLimit) {
	...
	public int getMaxPage() {
		/*
		 	...
		 	111 / 10 = 11.1		=> 12페이지
		 */
		return (int) Math.ceil((double) this.totalCount / this.listLimit);
	}
	...
	public int getEndList() {
		...
		return endList > this.totalCount ? this.totalCount : endList;
	}
}