...
<section>
    ...
    <table class="table table-hover text-center">
        ...
        <c:if test="${ empty searchreviewlist }">
            <tr>
                <td></td>
                <td></td>
        ...
        <c:if test="${ not empty searchreviewlist }">
    ...
    <table style="width: 100%; ">
        ...
        <tr>
            ...
            <th id="pageNumber"  style="width: 40%; text-align: center;">
                <p >
                <button type="button" class="btn btn-default btn-sm" onclick="location.href='${path}/board/reviewsearch?page=1'" >&lt;&lt;</button>
                <button type="button" class="btn btn-default btn-sm" onclick="location.href='${path}/board/reviewsearch?page=${ searchpageInfo.prevPage }'" >&lt;</button>
                <c:forEach begin="${ searchpageInfo.startPage }" end="${ searchpageInfo.endPage }" varStatus = "status" >
                	<c:choose>
	                	<c:when test="${ status.current == searchpageInfo.currentPage }">
							<button disabled class="btn btn-default btn-sm" >${ status.current }</button>
						</c:when>
						<c:otherwise>																												
																																			<%--${ status.current } --%>
		                	<button type="button" class="btn btn-default btn-sm" onclick="location.href='${path}/board/review?page=${ status.current }'" disabled >1</button>
						</c:otherwise>
                	</c:choose>

                </c:forEach>
                <button type="button" class="btn btn-default btn-sm" onclick="location.href='${path}/board/reviewsearch?page=${ searchpageInfo.nextPage }'" >&gt;</button>
                <button type="button" class="btn btn-default btn-sm" onclick="location.href='${path}/board/reviewsearch?page=${ searchpageInfo.maxPage }'" >&gt;&gt;</button>
                </p>
            </th>
            <td>
