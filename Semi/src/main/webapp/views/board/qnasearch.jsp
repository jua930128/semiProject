    ...
    <table class="table table-hover text-center">
        ...
        <tbody style="border-bottom: 1px solid gray; ">
        	...
        	<c:if test="${ empty searchqnalist }">
       		 ...
       		 <c:if test="${ not empty searchqnalist }">
       		 	<c:forEach var="qna" items="${ searchqnalist }" >
		            <tr>
		                <td>${ qna.no }</td>
    ...
    <table style="width: 100%; ">
            ...
            <tr>
                    ...
                    <th id="pageNumber"  style="width: 40%; text-align: center;">
                        <p >
                            <button type="button" class="btn btn-default btn-sm" onclick="location.href='${path}/board/qnasearch?page=1'" >&lt;&lt;</button>
                            <button type="button" class="btn btn-default btn-sm" onclick="location.href='${path}/board/qnasearch?page=${ searchpageInfo.prevPage }'" >&lt;</button>
<%--                             <c:forEach begin="${ pageInfo.startPage }" end="${ pageInfo.endPage }" varStatus = "status" > --%>
<%--                 				<c:choose> --%>
<%-- 	                			<c:when test="${ status.current == searchpageInfo.currentPage }"> --%>
									<button disabled class="btn btn-default btn-sm" >1</button>
<%-- 								</c:when> --%>
<%-- 								<c:otherwise> --%>
<%-- 		                			<button type="button" class="btn btn-default btn-sm" onclick="location.href='${path}/board/qnasearch?page=${ status.current }'" >${ status.current }</button> --%>
<%-- 								</c:otherwise> --%>
<%--                 				</c:choose> --%>
<%--                 			</c:forEach> --%>
                            <button type="button" class="btn btn-default btn-sm" onclick="location.href='${path}/board/qnasearch?page=${ searchpageInfo.nextPage }'" >&gt;</button>
                			<button type="button" class="btn btn-default btn-sm" onclick="location.href='${path}/board/qnasearch?page=${ searchpageInfo.maxPage }'" >&gt;&gt;</button>
                        </p>
                    </th>
                <td>
